...
├── src/
│   ├── module/             # MTA contract — the six entry points
│   ├── lua/                # Lua-stack helpers + typed binder
│   ├── registry/           # function registry + registration macros
│   ├── runtime/            # engine: scheduler, callback, resources, log
│   └── functions/          # 👉 YOUR functions, grouped by domain
├── tests/                  # embedded-Lua harness + .lua test scripts
├── docs/                   # this doc, API.md, GUIDES.md, TUTORIAL.md
└── vendor/                 # frozen third-party code (never edit)
...
```
  ...
  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐
  │   lua/       │  │  runtime/    │  │  module/     │
  │ binder, arg  │  │ scheduler,   │  │ MTA entry    │
  │ stack, user- │  │ callback,    │  │ points, info │
  │ data, events │  │ store, log   │  │              │
  └──────┬───────┘  └──────┬───────┘  └──────┬───────┘
         │                 │                 │
         └─────────────────┴───────┬─────────┘
                                   ▼
...
### 3.1 `src/module/` — the MTA contract
...
### 3.2 `src/registry/` — the function registry
...
### 3.3 `src/lua/` — stack helpers & the typed binder
...
- `bind.hpp` — the template machinery: reads typed parameters from the Lua
  stack, synthesizes missing optional ones, applies C++ defaults, dispatches
  on the reduced arity, and pushes results. This is where
  `args<double, double>(L)` comes from.
- `userdata.hpp` — `Registry<T>`: Lua objects as userdata with methods and
  a `__gc` destructor.
- `table_helpers.hpp` — `get_field`/`set_field`/`convert` for `Table`.
- `events.hpp` / `events.cpp` — `mta::events::trigger`: fires an MTA event
  into the resource's scripts (module → Lua).

### 3.4 `src/runtime/` — the engine

Background work and long-lived state, kept out of the per-call path:

- `scheduler` — a small thread pool (3 workers). `post_task(work,
  ...
  resource stops.
- `callback` — `mta::async::Callback`: a stable reference to a Lua function
  (`luaL_ref` + resource name). Survives `DoPulse` frames and resource
  restarts; never fires into a stopped resource.
- `resources` — `mta::resources::Hub` + `Store<T>`: per-resource data that is
  erased automatically on `ResourceStopped`. This is the safe replacement for
  "stored global state" in a multi-VM world.
- `logging` — `mta::log::{debug,info,warn,error}` with levels; goes to the
  MTA console via the manager, falls back to stdout in the harness.

### 3.5 `src/functions/` — your code
...
Everything below is a CMake cache variable / option:

| Variable | Default | Effect |
|---|---|---|
| `SDK_MODULE_NAME` | `base` | Output binary name → `base.dll` / `base.so` |
| `SDK_MODULE_TITLE` | `Base Module` | Module name shown in the server console |
| `SDK_MODULE_AUTHOR` | `anon` | Author shown in the server console |
| `SDK_UNITY` | `ON` | Unity build |
| `SDK_LTO` | `ON` | Link-time optimization |
...
The version comes from `project(VERSION 1.1.0)` — bump only there. The
module binary lands at `build/<preset>/module/<platform>-<arch>/<name>.dll`
(or `.so`).
...
- `tests/harness.cpp` boots a clean Lua 5.1 (the MTA-patched ABI, verified
  for the `mtasaowner` argument), installs a mock `ILuaModuleManager10`, and
  runs every `tests/scripts/*.lua` — no MTA server needed. Helpers:
...
- **New subsystems** get a namespace (`mta::<sub>`) and live in their own
  `src/<sub>/` folder with a `_test`-style coverage where practical.
